...
const TYPE_META: Record<string, { color: string; label: string }> = {
  "Fed":            { color: C.blue,   label: "Fed" },
  "Macro":          { color: C.orange, label: "Macro" },
  "Political":      { color: "#ff6b35",label: "Political" },
  "Earnings":       { color: C.cyan,   label: "Earnings" },
  "AI":             { color: "#5856d6",label: "AI" },
  "IPO":            { color: C.green,  label: "IPO" },
  "Banking Crisis": { color: "#ff2d55",label: "Banking" },
  "Geopolitical":   { color: "#8e8e93",label: "Geo" },
};
...
type TypeStat = { type: string; avg: number; vol: number; count: number; up: number; down: number };
type SummaryT = {
  total_days: number; event_days: number; avg_all: number;
  vol_event: number; vol_non_event: number;
  best: Row; worst: Row; big_moves: number;
};

function pctColor(p: number) {
  if (p >= 2) return C.green;
...
function PctBadge({ val }: { val: number | null }) {
  if (val == null) return <span style={{ color: C.faint }}>—</span>;
...
function StatCard({ label, val, sub, accent }: { label: string; val: string | number; sub?: string; accent?: string }) {
  return (
...
const TICKER_KEYS = ["QQQ", "SPY", "NDX"] as const;
const TABS = ["Timeline", "Event Analysis", "Top Moves", "Correlation"] as const;

const TICKER_ITEMS = [
  "857 Trading Days", "89 Annotated Events", "Event-Day Vol 2.55× Normal",
  "Best: QQQ +12.1% Apr 9 2025", "Worst: QQQ −5.0% Apr 8 2025",
  "AI Events: +0.47% avg", "Fed Days: coin flip", "Data: Yahoo Finance",
];

export default function MarketEventDashboard() {
  const [tab, setTab]           = useState<typeof TABS[number]>("Timeline");
  const [year, setYear]         = useState("ALL");
  const [ticker, setTicker]     = useState<"QQQ"|"SPY"|"NDX">("QQQ");
  ...
  const summary   = SUMMARY as SummaryT;

  const filtered = useMemo(() => {
    let r = [...rows];
    if (year !== "ALL") {
      if (year === "2025–2026") r = r.filter(x => x.date >= "2025");
      else r = r.filter(x => x.date.startsWith(year));
    }
    if (evOnly) r = r.filter(x => !!x.et);
    if (extremeOnly) r = r.filter(x => Math.abs(x[ticker.toLowerCase() as "qqq"] ?? x.qqq) >= 2);
    if (evFilter !== "ALL") r = r.filter(x => x.et === evFilter);
    if (search) r = r.filter(x => x.date.includes(search) || (x.en ?? "").toLowerCase().includes(search.toLowerCase()));
    return [...r].reverse();
  }, [rows, year, ticker, evOnly, extremeOnly, evFilter, search]);
  ...
  const monthlyHeatmap = useMemo(() => {
    ...
    return Object.entries(m).sort((a, b) => a[0].localeCompare(b[0])).map(([month, vals]) => ({
  ...
  const spreadByType = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      if (!m[r.et]) m[r.et] = [];
      m[r.et].push(r.qqq - r.spy);
    }
    return Object.entries(m).map(([et, vals]) => ({
      type: et,
      avg: Math.round(vals.reduce((a, b) => a + b, 0) / vals.length * 1000) / 1000,
      n: vals.length,
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "-apple-system, BlinkMacSystemFont, \'SF Pro Display\', \'Segoe UI\', sans-serif" }}>
      ...
      <div style={{ position: "relative", background: "#000", overflow: "hidden" }}>
        ...
        {/* Hero content */}
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "64px 32px 72px", position: "relative", zIndex: 2 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(0,113,227,0.12)", border: "1px solid rgba(0,113,227,0.35)", borderRadius: 20, padding: "5px 16px", marginBottom: 24 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 600, color: C.blue, letterSpacing: "0.02em" }}>VALUEADD VC · MARKET RESEARCH</span>
          </div>

          <h1 style={{ fontSize: "clamp(32px, 5vw, 58px)", fontWeight: 900, lineHeight: 1.04, letterSpacing: "-0.03em", color: "#f5f5f7", marginBottom: 18, maxWidth: 760 }}>
            <span className="shimmer-text">Market Event</span> Correlator
          </h1>
          <p style={{ fontSize: 17, color: "#8e8e93", lineHeight: 1.7, maxWidth: 600, marginBottom: 40 }}>
            857 trading days of QQQ, SPY, and Nasdaq-100 open→close returns, annotated with{" "}
            <span style={{ color: "#e5e5ea" }}>89 major market events</span>
            {" "}— Fed decisions, tariff shocks, AI launches, IPOs, and more.
          </p>

          {/* KPI cards */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12, maxWidth: 740 }}>
            {[
              { badge: "DATASET",  label: "Trading Days",       val: "857",    sub: "Jan 2023 – Jun 2026",         accent: C.blue   },
              { badge: "FINDING",  label: "Event-Day Volatility",val: "2.55×",  sub: "vs normal days (2.15% vs 0.84%)",accent: C.green  },
              { badge: "RECORD",   label: "Best Single Day",    val: "+12.1%", sub: "Apr 9 2025 — tariff pause",   accent: C.cyan   },
              { badge: "CATALYST", label: "AI Event Win Rate",  val: "89%",    sub: "8 of 9 AI events positive",   accent: C.orange },
            ].map(s => (
              <div key={s.label} style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", borderTop: `2px solid ${s.accent}`, borderRadius: 14, padding: "16px 18px" }}>
                <span style={{ fontSize: 9, fontWeight: 800, color: s.accent, letterSpacing: "0.1em", background: s.accent + "18", border: `1px solid ${s.accent}30`, borderRadius: 4, padding: "2px 6px", display: "inline-block", marginBottom: 10 }}>{s.badge}</span>
                <div className="mono" style={{ fontSize: 28, fontWeight: 800, color: "#f5f5f7", lineHeight: 1, marginBottom: 6 }}>{s.val}</div>
                <div style={{ fontSize: 11, color: "#8e8e93", fontWeight: 600, marginBottom: 3 }}>{s.label}</div>
      ...
      <div className="nav-glass" style={{ position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", gap: 4, height: 52, overflowX: "auto" }}>
          {TABS.map(t => (
            <button key={t} onClick={() => setTab(t)} style={{
              color: tab === t ? "#f5f5f7" : "#8e8e93",
          ...
          <div style={{ flex: 1 }} />
          <span style={{ fontSize: 11, color: "#6e6e73" }}>QQQ · SPY · NDX 2023–2026</span>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px 80px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px,1fr))", gap: 12, marginBottom: 28 }}>
          <StatCard label="Total Days"      val={summary.total_days}                                               accent={C.blue}   sub="Jan 2023 – Jun 2026" />
          <StatCard label="Event Days"      val={summary.event_days}                                               accent={C.blue}   sub={`${((summary.event_days/summary.total_days)*100).toFixed(1)}% of sessions`} />
          <StatCard label="Avg Return/Day"  val={(summary.avg_all >= 0 ? "+" : "") + summary.avg_all.toFixed(3) + "%"} accent={summary.avg_all >= 0 ? C.green : C.red} />
          <StatCard label="Event-Day Vol"   val={summary.vol_event.toFixed(2) + "%"}                              accent={C.orange} sub="±σ open→close" />
          <StatCard label="Normal-Day Vol"  val={summary.vol_non_event.toFixed(2) + "%"}                          accent={C.cyan}   sub={`${(summary.vol_event/summary.vol_non_event).toFixed(2)}× lower`} />
          <StatCard label="Best Day (QQQ)"  val={"+" + summary.best.qqq.toFixed(2) + "%"}                         accent={C.green}  sub={summary.best.date} />
          <StatCard label="Worst Day (QQQ)" val={summary.worst.qqq.toFixed(2) + "%"}                              accent={C.red}    sub={summary.worst.date} />
          <StatCard label="±2%+ Days"       val={summary.big_moves}                                               accent={C.orange} sub={`${((summary.big_moves/summary.total_days)*100).toFixed(1)}% of days`} />
        </div>
        ...
        {tab === "Timeline" && (
          <div>
            {/* Filters */}
            <div className="card-lift" style={{ background: C.card, borderRadius: 14, boxShadow: "0 2px 8px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.04)", padding: "16px 20px", marginBottom: 16, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
              {YEAR_FILTERS.map(y => (
                <button key={y} onClick={() => { setYear(y); setPage(0); }} style={{
                  padding: "5px 12px", borderRadius: 20, border: `1px solid ${year === y ? C.blue : "rgba(0,0,0,0.12)"}`,
                  background: year === y ? C.blue + "12" : "transparent",
                  color: year === y ? C.blue : C.dim, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit",
                }}>{y}</button>
              ))}
              <div style={{ width: 1, height: 20, background: "rgba(0,0,0,0.1)", margin: "0 4px" }} />
              {TICKER_KEYS.map(tk => (
                <button key={tk} onClick={() => setTicker(tk)} style={{
                  padding: "5px 12px", borderRadius: 20, border: `1px solid ${ticker === tk ? C.text : "rgba(0,0,0,0.12)"}`,
                  background: ticker === tk ? "rgba(0,0,0,0.06)" : "transparent",
                  color: ticker === tk ? C.text : C.dim, fontSize: 12, fontWeight: ticker === tk ? 700 : 600, cursor: "pointer", fontFamily: "inherit",
                }}>{tk}</button>
              ))}
              <div style={{ width: 1, height: 20, background: "rgba(0,0,0,0.1)", margin: "0 4px" }} />
              {Object.entries(TYPE_META).map(([et, m]) => (
                <button key={et} onClick={() => { setEvFilter(evFilter === et ? "ALL" : et); setPage(0); }} style={{
                  padding: "4px 10px", borderRadius: 20, fontSize: 11,
                  border: `1px solid ${evFilter === et ? m.color : "rgba(0,0,0,0.10)"}`,
                  background: evFilter === et ? m.color + "12" : "transparent",
                  color: evFilter === et ? m.color : C.dim, fontWeight: 600, cursor: "pointer", fontFamily: "inherit",
                }}>{m.label}</button>
              ))}
              <div style={{ width: 1, height: 20, background: "rgba(0,0,0,0.1)", margin: "0 4px" }} />
              ...
              <input placeholder="Search date or event…" value={search} onChange={e => { setSearch(e.target.value); setPage(0); }} style={{
                ...
                fontSize: 12, outline: "none", fontFamily: "inherit", color: C.text, background: "#f5f5f7", width: 180,
              }} />
              <div style={{ marginLeft: "auto", fontSize: 12, color: C.dim, fontWeight: 600 }}>{filtered.length} days</div>
            </div>

            {/* Table */}
            <div className="card-lift" style={{ background: C.card, borderRadius: 14, boxShadow: "0 2px 8px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.04)", overflow: "hidden" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ borderBottom: "1px solid rgba(0,0,0,0.08)" }}>
                    {["Date", "Type", "QQQ", "SPY", "NDX", "Move", "Event"].map(h => (
                      <th key={h} style={{ padding: "10px 14px", textAlign: "left", color: C.dim, fontWeight: 700, fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em", whiteSpace: "nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {paginated.length === 0 && (
                    <tr><td colSpan={7} style={{ padding: 40, textAlign: "center", color: C.faint, fontSize: 13 }}>No data for selected filters.</td></tr>
                  )}
                  {paginated.map((row, i) => {
                    ...
                    return (
                      <tr key={row.date} className="tr-hover" style={{
                        ...
                        <td className="mono" style={{ padding: "9px 14px", color: C.dim, fontSize: 12, whiteSpace: "nowrap" }}>{row.date}</td>
                        ...
                        <td style={{ padding: "9px 14px" }}><PctBadge val={row.qqq} /></td>
                        <td style={{ padding: "9px 14px" }}><PctBadge val={row.spy ?? null} /></td>
                        <td style={{ padding: "9px 14px" }}><PctBadge val={row.ndx ?? null} /></td>
                        <td style={{ padding: "9px 14px" }}><MiniBar pct={row.qqq} /></td>
                        <td style={{ padding: "9px 14px", maxWidth: 360 }}>
                          {row.en && (
                            <>
                              <div style={{ fontWeight: 700, fontSize: 12, color: m?.color ?? C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{row.en}</div>
                              <div style={{ fontSize: 11, color: C.dim, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 340 }}>{row.ed?.slice(0, 100)}</div>
                            </>
                          )}
                        </td>
                      </tr>
            ...
            {totalPages > 1 && (
              <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 8, marginTop: 16 }}>
                <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} style={{
                  padding: "6px 16px", borderRadius: 20, border: "1px solid rgba(0,0,0,0.12)", background: page === 0 ? "#f5f5f7" : "#fff",
                  color: page === 0 ? C.faint : C.text, cursor: page === 0 ? "not-allowed" : "pointer", fontSize: 12, fontFamily: "inherit", fontWeight: 600,
                }}>← Prev</button>
                <span style={{ fontSize: 12, color: C.dim }}>{page + 1} / {totalPages} · {filtered.length} rows</span>
                <button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page === totalPages - 1} style={{
                  padding: "6px 16px", borderRadius: 20, border: "1px solid rgba(0,0,0,0.12)", background: page === totalPages - 1 ? "#f5f5f7" : "#fff",
                  color: page === totalPages - 1 ? C.faint : C.text, cursor: page === totalPages - 1 ? "not-allowed" : "pointer", fontSize: 12, fontFamily: "inherit", fontWeight: 600,
                }}>Next →</button>
              </div>
            )}
          </div>
        ...
        {tab === "Event Analysis" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            {/* Avg return by type — bar chart */}
            <div className="card-lift" style={{ background: C.card, borderRadius: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.04)", padding: 24, borderTop: `2px solid ${C.blue}` }}>
              <div style={{ fontWeight: 700, fontSize: 15, color: C.text, marginBottom: 4 }}>Avg QQQ Return by Event Type</div>
              <div style={{ fontSize: 13, color: C.dim, marginBottom: 20 }}>Open-to-close return on event days vs category. AI events are the strongest catalyst; Geopolitical and Macro skew negative.</div>
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={[...typeStats].sort((a, b) => b.avg - a.avg)} margin={{ top: 4, right: 8, bottom: 4, left: 8 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(0,0,0,0.05)" />
                  <XAxis dataKey="type" tick={{ fill: C.dim, fontSize: 11 }} />
                  <YAxis tick={{ fill: C.dim, fontSize: 11 }} tickFormatter={v => `${v >= 0 ? "+" : ""}${v}%`} />
                  <Tooltip contentStyle={{ background: "#fff", border: "1px solid rgba(0,0,0,0.1)", borderRadius: 10, fontSize: 12 }}
                    formatter={(v: unknown) => { const n = v as number; return [`${n >= 0 ? "+" : ""}${n.toFixed(3)}%`, "Avg QQQ Return"]; }} />
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px,1fr))", gap: 14 }}>
              {[...typeStats].sort((a, b) => b.avg - a.avg).map(ts => {
                const m = TYPE_META[ts.type] ?? { color: C.blue };
                return (
                  <div key={ts.type} className="card-lift" style={{ background: C.card, borderRadius: 14, boxShadow: "0 2px 8px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.04)", padding: "18px 20px", borderTop: `2px solid ${m.color}` }}>
                    <div style={{ fontSize: 11, color: C.dim, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8, fontWeight: 700 }}>{ts.type}</div>
                    <div className="mono" style={{ fontSize: 30, fontWeight: 800, color: ts.avg >= 0 ? C.green : C.red, lineHeight: 1, marginBottom: 6 }}>
                      {ts.avg >= 0 ? "+" : ""}{ts.avg.toFixed(3)}%
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginTop: 10, borderTop: "1px solid rgba(0,0,0,0.05)", paddingTop: 10 }}>
                      <div><div style={{ fontSize: 10, color: C.dim }}>Win Rate</div><div className="mono" style={{ fontWeight: 700, color: (ts.up / ts.count) >= 0.5 ? C.green : C.red }}>{Math.round(ts.up / ts.count * 100)}%</div></div>
                      <div><div style={{ fontSize: 10, color: C.dim }}>Volatility</div><div className="mono" style={{ fontWeight: 700, color: C.dim }}>σ {ts.vol.toFixed(2)}%</div></div>
                      <div><div style={{ fontSize: 10, color: C.dim }}>Events</div><div className="mono" style={{ fontWeight: 700 }}>{ts.count}</div></div>
                      <div><div style={{ fontSize: 10, color: C.dim }}>↑ / ↓</div><div className="mono" style={{ fontWeight: 700 }}><span style={{ color: C.green }}>{ts.up}</span> / <span style={{ color: C.red }}>{ts.down}</span></div></div>
                    </div>
                  </div>
                );
              })}
            ...
            {/* Key findings */}
            <div className="card-lift" style={{ background: C.card, borderRadius: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.04)", padding: 24, borderTop: `2px solid ${C.green}` }}>
              <div style={{ fontWeight: 700, fontSize: 15, color: C.text, marginBottom: 16 }}>Key Findings</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
                {[
                  { color: "#5856d6", text: "AI announcements are the strongest catalyst: avg +0.46% QQQ, 8/9 events positive. DeepSeek (Jan 27 2025) was the only exception — NVDA dragged the index −3.8% intraday." },
                  { color: "#ff2d55", text: "Banking crisis days (SVB, Signature Bank) average +0.96% — markets front-run FDIC/Fed bailout announcements and relief bounces faster than the initial selloff." },
                  { color: C.blue,    text: "Fed days are a perfect coin flip: avg +0.07%, exactly 7 up / 7 down. Direction depends entirely on wording vs market expectations, not the actual decision." },
                  { color: C.orange,  text: "Macro data days (CPI, jobs reports) skew negative: avg −0.48%. Upside surprises trigger rate-hike fears; misses trigger recession fears. Either way, tech loses." },
                  { color: "#ff6b35", text: "Political events have the highest volatility (σ=4.32%) — the Apr 9 tariff pause (+12.1%) and Apr 8 tariff escalation (−5.0%) are in the same category. Direction is unpredictable; magnitude is guaranteed." },
                  { color: C.green,   text: "IPO days have a slight negative drag (−0.20%). Large offerings absorb secondary market capital; they also tend to cluster during risk-off windows (Birkenstock, CoreWeave both disappointed)." },
                  { color: C.text,    text: "Core insight: event days have 2.55× higher volatility than non-event days (2.15% vs 0.84% σ). Accurately predicting event direction yields dramatically larger payoffs than random trading." },
                ].map((ins, i, arr) => (
                  <div key={i} style={{ display: "flex", gap: 14, padding: "14px 0", borderBottom: i < arr.length - 1 ? "1px solid rgba(0,0,0,0.05)" : "none" }}>
                    <div style={{ width: 3, minWidth: 3, borderRadius: 2, background: ins.color, alignSelf: "stretch" }} />
                    <div style={{ fontSize: 13, color: C.dim, lineHeight: 1.65 }}>{ins.text}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {tab === "Top Moves" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            {(["BEST", "WORST"] as const).map(dir => {
              ...
              const accent = dir === "BEST" ? C.green : C.red;
              return (
                <div key={dir} className="card-lift" style={{ background: C.card, borderRadius: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.04)", overflow: "hidden", borderTop: `2px solid ${accent}` }}>
                  <div style={{ padding: "18px 24px", borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
                    <div style={{ fontWeight: 700, fontSize: 15, color: C.text }}>Top 20 {dir === "BEST" ? "Best" : "Worst"} Days — QQQ</div>
                    <div style={{ fontSize: 12, color: C.dim, marginTop: 2 }}>Open-to-close return with event annotation where available</div>
                  </div>
                        ...
                        {sorted.map((row, i) => {
                          ...
                          return (
                            <tr key={row.date} className="tr-hover" style={{
                              ...
                              <td className="mono" style={{ padding: "9px 14px", color: C.faint, fontSize: 11 }}>{i + 1}</td>
                              <td className="mono" style={{ padding: "9px 14px", color: C.dim, whiteSpace: "nowrap" }}>{row.date}</td>
                              <td style={{ padding: "9px 14px" }}><PctBadge val={row.qqq} /></td>
                              <td style={{ padding: "9px 14px" }}><PctBadge val={row.spy ?? null} /></td>
                              <td style={{ padding: "9px 14px" }}><PctBadge val={row.ndx ?? null} /></td>
                              <td style={{ padding: "9px 14px" }}>
                                {m && <span style={{ fontSize: 10, fontWeight: 700, color: m.color, background: m.color + "15", border: `1px solid ${m.color}30`, borderRadius: 20, padding: "2px 8px" }}>{m.label}</span>}
                              </td>
                              <td style={{ padding: "9px 14px", maxWidth: 320 }}>
                                {row.en
                                  ? <span style={{ fontWeight: 600, color: C.text }}>{row.en}</span>
                                  : <span style={{ color: C.faint }}>No event tagged</span>
                                }
                              </td>
                            </tr>
                          );
        ...
        {tab === "Correlation" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            {/* QQQ–SPY spread by event type */}
            <div className="card-lift" style={{ background: C.card, borderRadius: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.04)", padding: 24, borderTop: `2px solid ${C.cyan}` }}>
              <div style={{ fontWeight: 700, fontSize: 15, color: C.text, marginBottom: 4 }}>QQQ − SPY Spread by Event Type</div>
              <div style={{ fontSize: 13, color: C.dim, marginBottom: 20 }}>Positive = tech outperformed broad market on that event type. AI and Earnings events drive the strongest tech outperformance; Macro events show defensive rotation away from tech.</div>
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={spreadByType} margin={{ top: 4, right: 8, bottom: 4, left: 8 }}>
                  ...
                  <YAxis tick={{ fill: C.dim, fontSize: 11 }} tickFormatter={v => `${v >= 0 ? "+" : ""}${v}%`} />
                  <Tooltip contentStyle={{ background: "#fff", border: "1px solid rgba(0,0,0,0.1)", borderRadius: 10, fontSize: 12 }}
                    formatter={(v: unknown) => { const n = v as number; return [`${n >= 0 ? "+" : ""}${n.toFixed(3)}%`, "QQQ−SPY Spread"]; }} />
                  <ReferenceLine y={0} stroke={C.dim} strokeWidth={1} />
                  <Bar dataKey="avg" radius={[5, 5, 0, 0]}>
                    {spreadByType.map(d => (
            ...
            {/* Monthly heatmap */}
            <div className="card-lift" style={{ background: C.card, borderRadius: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.04)", padding: 24, borderTop: `2px solid ${C.blue}` }}>
              <div style={{ fontWeight: 700, fontSize: 15, color: C.text, marginBottom: 4 }}>Monthly Average Daily QQQ Return</div>
              <div style={{ fontSize: 13, color: C.dim, marginBottom: 20 }}>Each tile = avg open→close return for that month across all trading days. April 2025 is a clear outlier (tariff shock + 90-day pause in the same month).</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {monthlyHeatmap.map(m => {
                  const intensity = Math.min(Math.abs(m.avg) / 1.5, 1);
                  const bg = m.avg >= 0 ? `rgba(52,199,89,${intensity * 0.65})` : `rgba(255,59,48,${intensity * 0.55})`;
                  return (
                    <div key={m.month} title={`${m.fullMonth}: avg ${m.avg > 0 ? "+" : ""}${m.avg}%/day`} style={{
                      width: 60, padding: "8px 6px", background: bg,
                      border: "1px solid rgba(0,0,0,0.07)", borderRadius: 8, textAlign: "center",
                    }}>
                      <div style={{ fontSize: 9, color: "rgba(0,0,0,0.45)", letterSpacing: "0.04em", marginBottom: 3 }}>{m.month}</div>
                      <div className="mono" style={{ fontSize: 11, fontWeight: 800, color: m.avg >= 0 ? "#1a7f3c" : "#c0392b" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              {[
                { label: "Event Days", vol: summary.vol_event, color: C.orange, count: summary.event_days, note: "Fed, earnings, tariffs, AI launches…" },
                { label: "Non-Event Days", vol: summary.vol_non_event, color: C.cyan, count: summary.total_days - summary.event_days, note: "Typical market sessions" },
              ].map(v => (
                <div key={v.label} className="card-lift" style={{ background: C.card, borderRadius: 14, boxShadow: "0 2px 8px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.04)", padding: 22, borderTop: `2px solid ${v.color}` }}>
                  <div style={{ fontSize: 11, color: C.dim, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8 }}>{v.label}</div>
                  <div className="mono" style={{ fontSize: 36, fontWeight: 800, color: v.color, lineHeight: 1 }}>σ {v.vol.toFixed(2)}%</div>
                  <div style={{ fontSize: 12, color: C.dim, marginTop: 8 }}>{v.count} sessions · {v.note}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ marginTop: 48, paddingTop: 24, borderTop: "1px solid rgba(0,0,0,0.08)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          <div style={{ fontSize: 11, color: C.faint }}>Data: Yahoo Finance · Open→Close intraday return · Not adjusted for dividends · Informational use only</div>
